...
def converter_form_umid(df, col_umid = "Umidade"):
    padrao = '\d+,\d+'
    if df[col_umid].dtype == 'str':
        for i in range(len(df[col_umid])):
            if re.match(padrao, df.loc[i, col_umid]):
                t1, t2 = (df.loc[i, col_umid]).split(',')
                df.loc[i, col_umid] = t1 + '.' + t2
            
    df = df.astype({col_umid: float})
...
def tratar_data_umid(df, col_data, ano = '2021'):
    datas = []
...
def req_umid_inmet(data, hora):
    url_api = "https://apitempo.inmet.gov.br/estacao/" + data + "/" + data + "/" + "A557#"
    r = requests.get(url = url_api)
    d = r.json()
    
    return d[int(hora)]['UMD_INS']

#Gera um dicionario dentro de uma lista a partir dos dados de cada linha do dataset
def gerar_dict_linha(df, linha, col_data = 'Data', col_hora = 'Hora', col_umid = 'Umidade', umid_ext = None):
    d = {"date":None, "horario":None, "humidityInterna":None, "humidityExterna":None}
    l = []
    
    d["date"] = df.loc[linha, col_data]
    d["horario"] = df.loc[linha, col_hora]
    d["humidityInterna"] = df.loc[linha, col_umid]

    if umid_ext == None:
        horario_separado = (str(d["horario"])).split(':', 1)
        hora = horario_separado[0]
        umid_ext_str = req_umid_inmet(str(d["date"]), hora)
        if type(umid_ext_str) != 'string' and type(umid_ext_str) != 'float' and type(umid_ext_str) != 'int':
            umid_externa = None
        else:
            umid_externa = float(req_umid_inmet(str(d["date"]), hora))
    else:
        umid_externa = umid_ext

    d["humidityExterna"] = umid_externa
    
    l.append(d)
    return l

#Gera um dicionario com todos os dicionarios gerados a partir de cada linha do dataset
def gerar_grupo_dicts(df, col_data = 'Data', col_hora = 'Hora', col_umid = 'Umidade'):
    l = []
    l_umid = []
    umid_ext = 0
    hora_ant = "25"
    ...
    for i in range(len(df.index)):
        ...
        hora = horario_separado[0]
        if hora == hora_ant:
            l += gerar_dict_linha(df, i, col_data, col_hora, col_umid, umid_ext)
        else:
            l_umid = gerar_dict_linha(df, i, col_data, col_hora, col_umid)
            l += l_umid
            umid_ext = l_umid[0]["humidityExterna"]
            hora_ant = hora
        interations = interations + 1
        print("building...",  "{:10.2f}".format(interations*100/len(df.index)), "%")
