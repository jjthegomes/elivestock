...
export const getHora = (data) =>
  getData(data).getHours().toString().length === 1
    ? "0" + getData(data).getHours()
    : getData(data).getHours();

export const getMinutos = (data) =>
  getData(data).getMinutes().toString().length === 1
    ? "0" + getData(data).getMinutes()
    : getData(data).getMinutes();
...
export const getDataFormatada = (data) => {
  ...
  const dia =
    data.getUTCDate() >= 10 ? data.getUTCDate() : "0" + data.getUTCDate();
  return ano + "-" + mes + "-" + dia;
};
...
export const getTempoEspera = (milisegundo, data) => {
  ...
  const msToTime = (duration) => {
    ...
    return `${hours} ${hours === "01" ? "hora" : "horas"} e ${minutes} ${
